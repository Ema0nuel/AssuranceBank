import{s as n}from"./supabaseClient-BzAKCqO5.js";import{A as k}from"./AdminNavbar-D-kZqzSj.js";import{r as E}from"./adminAuth-WhKi00aa.js";import{s as u}from"./toast-DRvdR0y9.js";import{s as v}from"./sendEmail-89Z52C2k.js";import{s as $}from"./signupHandler-T-aL-TRc.js";import"./logo-nobg-UuYbhWoI.js";import"./logo-B-uS21Zh.js";import"./email-Bcw4L-Oi.js";const f="eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJpc3MiOiJzdXBhYmFzZSIsInJlZiI6Im1oZnNvemNnaXZhZHF0bXZ1cm1oIiwicm9sZSI6InNlcnZpY2Vfcm9sZSIsImlhdCI6MTc2NTM5NDA5OCwiZXhwIjoyMDgwOTcwMDk4fQ.Jxqc2Zf0y0zK6H84pC-_RewiByzG6e7p5NFVLrXhM6o",I="https://ifodbqygscdsxxlxfjxw.supabase.co";async function S(l){return(await fetch(`${I}/auth/v1/admin/users/${l}`,{method:"DELETE",headers:{apikey:f,Authorization:`Bearer ${f}`,"Content-Type":"application/json"}})).ok}function x(){return`<div class="flex justify-center items-center py-16">
  ...
  `}const O=async()=>{if(!await E())return{html:"",pageEvents:()=>{}};let l="users",e=!1,d=localStorage.getItem("admin_dark")==="true",{data:_=[]}=await n.from("profiles").select("*").order("created_at",{ascending:!1}),{data:h=[]}=await n.from("accounts").select("*"),{data:w=[]}=await n.from("crypto_balances").select("*"),m=_.map(p=>{const c=h.find(s=>s.user_id===p.id)||{},i=w.find(s=>s.user_id===p.id)||{};return{...p,account_type:c.account_type||"-",account_id:c.id,is_active:c.is_active!==!1&&p.is_active!==!1,balance:parseFloat(c.balance||0).toFixed(2),mortgage:parseFloat(c.mortgage||0).toFixed(2),loan:parseFloat(c.loan||0).toFixed(2),btc_balance:parseFloat(i.btc_balance||0).toFixed(8),eth_balance:parseFloat(i.eth_balance||0).toFixed(8),usdt_balance:parseFloat(i.usdt_balance||0).toFixed(6),usdc_balance:parseFloat(i.usdc_balance||0).toFixed(6),bnb_balance:parseFloat(i.bnb_balance||0).toFixed(8),sol_balance:parseFloat(i.sol_balance||0).toFixed(8)}}),y=m;function r(){document.getElementById("app").innerHTML=`
    ${k({activeItem:l,isCollapsed:e,isDark:d})}
