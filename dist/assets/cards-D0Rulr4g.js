import{s as c}from"./supabaseClient-BzAKCqO5.js";import{n as _}from"./Navbar-pMjcO_a8.js";import{s as g}from"./toast-DRvdR0y9.js";import{r as $}from"./reset-CYKpHJhn.js";import"./logo-B-uS21Zh.js";function E(){let t="";for(let a=0;a<16;a++)t+=Math.floor(Math.random()*10);return t.replace(/(.{4})/g,"$1 ").trim()}function S(){return""+Math.floor(100+Math.random()*900)}function L(){const t=new Date,a=t.getFullYear()+4;return`${(t.getMonth()+1).toString().padStart(2,"0")}/${a.toString().slice(-2)}`}function w({card_number:t,card_type:a,card_holder:i,expiry:n,cvv:v}){const o=a==="visa"?'<span class="font-bold italic tracking-tighter text-white text-2xl">VISA</span>':`<div class="flex items-center space-x-1">
        ...
        <div class="card-face card-back rounded-2xl select-none">
          ...
          <div class="mx-6 mt-8">
            ...
            <div class="mt-6 text-xs text-white/60 leading-relaxed">
              This card is property of Assurance Bank. Unauthorized use is prohibited. 
  ...
  `}function k(t){if(!t)return"";const a=new Date(t);return`${a.getMonth()+1}/${a.getFullYear().toString().slice(-2)}`}const j=async()=>{$("Assurance Bank | Cards");const t=_(),a=await c.auth.getSession();if(!a.data.session){window.location.href="/login";return}const{user:i}=a.data.session,{data:n}=await c.from("profiles").select("*").eq("id",i.id).single();let{data:v}=await c.from("accounts").select("*").eq("user_id",i.id).single(),{data:o}=await c.from("cards").select("*").eq("user_id",i.id).order("issued_at",{ascending:!1});const f=(o||[]).length?o.map((d,m)=>`
        ...
        <div class="bg-gray-50 dark:bg-gray-900 font-sans min-h-screen pt-12">
          <div id="main-content" class="ml-56 pt-14 transition-all duration-300 font-sans min-h-screen">
            ...
            <footer class="p-4 text-center text-gray-600 dark:text-gray-400 text-xs">
              <p>
                <strong>Copyright ï¿½ ${new Date().getFullYear()}</strong> All rights reserved | Assurance Bank.
              </p>
            </footer>
          </div>
        </div>
