import{s as o}from"./supabaseClient-BzAKCqO5.js";import{n as D}from"./Navbar-pMjcO_a8.js";import{s as n}from"./toast-DRvdR0y9.js";import{r as R}from"./reset-CYKpHJhn.js";import"./logo-B-uS21Zh.js";function y(a){if(!a)return"";const t=new Date(a);return`${t.getMonth()+1}/${t.getFullYear().toString().slice(-2)}`}function S(){return(Math.random()*4+6).toFixed(2)}function B(a){const t=new Date;return t.setMonth(t.getMonth()+a),t.toISOString().slice(0,10)}const P=async()=>{R("Assurance Bank | Loan");const a=D(),t=await o.auth.getSession();if(!t.data.session){window.location.href="/login";return}const{user:r}=t.data.session,{data:m}=await o.from("profiles").select("*").eq("id",r.id).single();let{data:h}=await o.from("accounts").select("*").eq("user_id",r.id).single(),{data:x}=await o.from("loan").select("*").eq("user_id",r.id).order("created_at",{ascending:!1});const v=(x||[]).length?x.map(e=>`
    ...
    `,pageEvents:w}};export{P as default};
