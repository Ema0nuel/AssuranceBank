import{s as r}from"./supabaseClient-BzAKCqO5.js";import{s as g}from"./toast-DRvdR0y9.js";import{s as h}from"./sendEmail-89Z52C2k.js";import{r as w}from"./reset-CYKpHJhn.js";import{n as k}from"./Navbar-pMjcO_a8.js";import"./logo-B-uS21Zh.js";const E="assurancebankcc@gmail.com",S=async()=>{const f=k();w("Assurance Bank | Contact Us");const m=await r.auth.getSession();if(!m.data.session){window.location.href="/login";return}const{user:u}=m.data.session,{data:b}=await r.from("profiles").select("*").eq("id",u.id).single();let d=[];async function x(){const{data:s}=await r.from("notifications").select("*").eq("user_id",u.id).order("created_at",{ascending:!1}).limit(20);d=s||[]}function p(){const s=document.getElementById("notif-view");if(!s)return;s.innerHTML=`
