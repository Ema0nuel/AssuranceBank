import{s as u}from"./supabaseClient-BzAKCqO5.js";import{n as T}from"./Navbar-pMjcO_a8.js";import{s as o}from"./toast-DRvdR0y9.js";import{r as $}from"./reset-CYKpHJhn.js";import{N}from"./logo-nobg-UuYbhWoI.js";import"./logo-B-uS21Zh.js";const I=["E9876567","G0876578","8767898H","K2387651","456L7890","1M234567","987654N3","O2345678","98765P43","Q1987654","R7654321","567S1234","T3456789","876543U1","V1234567","345678W9","X2345678","9Y876543","Z3456781","234567A8","B3456781","123456C7","D2345678","876E5432","5678F123","6789G123","7890H234","8901I345","9012J456","0123K567","K4561237","L5672348","M6783459","N7894560","O8905671","P9016782","Q0127893","R1238904","S2349015","T3450126","U4561237","V5672348","W6783459","X7894560","Y8905671","Z9016782","A0127893","B1238904","C2349015","D3450126","E4561237","F5672348","G6783459","H7894560","I8905671","J9016782","K0127893","L1238904","M2349015","N3450126","O4561237","P5672348","Q6783459","R7894560","S8905671","T9016782","U0127893","V1238904","W2349015","X3450126","Y4561237","Z5672348","A6783459","B7894560","C8905671","D9016782","E0127893","F1238904","G2349015","H3450126","I4561237","J5672348","K6783459","L7894560","M8905671","N9016782","O0127893","P1238904","Q2349015","R3450126","S4561237","T5672348","U6783459","V7894560","W8905671","X9016782","Y0127893","Z1238904","A2349015","B3450126","C4561237","D5672348","E6783459","F7894560","G8905671","H9016782","I0127893","J1238904","K2349015","L3450126","M4561237","N5672348","O6783459","P7894560","Q8905671","R9016782","S0127893","T1238904","U2349015","V3450126","W4561237","X5672348","Y6783459","Z7894560","A8905671","B9016782","C0127893","D1238904","E2349015","F3450126","G4561237","H5672348","I6783459","J7894560","K8905671","L9016782","M0127893","N1238904","O2349015","P3450126"];function A(){return Math.floor(1e5+Math.random()*9e5).toString()}async function B(){try{return await(await fetch("https://ipapi.co/json/")).json()}catch{return{}}}function y(a={}){const e={...{title:"Inter-Bank Transfer Receipt",receiptId:x(),date:new Date().toLocaleDateString(),time:new Date().toLocaleTimeString(),amount:"0.00",currency:"$",description:"Inter-Bank Transfer",senderName:"",recipientName:"",bankName:"",accountNumber:"",transactionType:"Inter-Bank Transfer",status:"Pending",referenceNumber:"",fees:"0.00",totalAmount:"",companyName:"Assurance Bank",companyAddress:"123 Main St, City, Country",companyPhone:"+1 (555) 123-4567",companyEmail:"assurancebankcc@gmail.com",additionalFields:{},showFooter:!0,footerText:"Thank you for banking with us!"},...a};if(!e.totalAmount){const r=parseFloat(e.amount)||0,n=parseFloat(e.fees)||0;e.totalAmount=(r+n).toFixed(2)}return`
    <div class="receipt-container font-mono">
      <div class="text-center mb-4">
        <img src="${N}" alt="Assurance Bank" class="h-10 mx-auto mb-2" />
    ...
    `}function x(){const a=Date.now().toString().slice(-6),d=Math.random().toString(36).substr(2,4).toUpperCase();return`RCP-${a}-${d}`}function g(a,d){let e=document.getElementById("code-modal");e||(e=document.createElement("div"),e.id="code-modal",document.body.appendChild(e)),e.className="",e.innerHTML=`
  ...
  `,document.getElementById("close-code-modal").onclick=()=>{e.innerHTML="",e.className="hidden"},document.getElementById("code-form").onsubmit=async function(r){r.preventDefault();const n=this.code.value.trim();I.includes(n)?(e.innerHTML="",e.className="hidden",d()):o("Invalid code. Please contact support.","error")}}function P(){let a=document.getElementById("success-modal");a||(a=document.createElement("div"),a.id="success-modal",document.body.appendChild(a)),a.className="",a.innerHTML=`
  ...
  `;const e=document.getElementById("success-canvas").getContext("2d");let r=0;function n(){e.clearRect(0,0,120,120),e.lineWidth=6,e.strokeStyle="#16a34a",e.beginPath(),e.arc(60,60,48,Math.PI*.5,Math.PI*(2*r),!1),e.stroke(),r<1?(r+=.03,requestAnimationFrame(n)):(e.beginPath(),e.moveTo(40,65),e.lineTo(55,80),e.lineTo(85,45),e.strokeStyle="#16a34a",e.lineWidth=8,e.stroke())}n(),document.getElementById("close-success-modal").onclick=()=>{a.innerHTML="",a.className="hidden",window.location.reload()}}const M=async()=>{$("Assurance Bank | Inter-Bank Transfer");const a=T(),d=await u.auth.getSession();if(!d.data.session){window.location.href="/login";return}const{user:e}=d.data.session,{data:r}=await u.from("profiles").select("*").eq("id",e.id).single();let{data:n}=await u.from("accounts").select("*").eq("user_id",e.id).single();const f=b=>typeof b=="number"?b.toLocaleString("en-US",{style:"currency",currency:"USD",minimumFractionDigits:2}):b||"$0.00";function v(){a.pageEvents?.(),document.getElementById("interbank-transfer-form").onsubmit=async function(t){t.preventDefault();try{const s=parseFloat(this.amount.value),i=this.accountName.value.trim(),l=this.accountNum.value.trim(),c=this.desc.value.trim();if(!s||!i||!l||!c){o("All fields are required.","error");return}if(s<=0){o("Amount must be greater than zero.","error");return}if(s>n.balance){o("Insufficient balance.","error");return}o("Sending OTP...","info");const m=A(),p=await B(),{error:k}=await u.from("otps").insert([{user_id:e.id,code:m,type:"interbank",expires_at:new Date(Date.now()+10*6e4).toISOString()}]);if(k){o("Database error. Please try again.","error");return}b({amount:s,accountName:i,accountNum:l,desc:c,profile:r,account:n,otp:m,ipLoc:p}),fetch("/api/send-email",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({to:e.email,subject:"Your OTP for Inter-Bank Transfer",html:`
      ...
      `,document.getElementById("close-otp-modal").onclick=()=>{s.innerHTML="",s.className="hidden"},document.getElementById("otp-form").onsubmit=async function(i){i.preventDefault();const l=this.otp.value.trim(),{data:c,error:m}=await u.from("otps").select("*").eq("user_id",t.profile.id).eq("code",l).eq("type","interbank").order("created_at",{ascending:!1}).limit(1).single();if(m||!c||new Date(c.expires_at)<new Date){o("Invalid or expired OTP.","error");return}h(t)}}function h(t){let s=document.getElementById("otp-modal");s.className="",s.innerHTML=`
      ...
      `,document.getElementById("close-receipt-modal").onclick=()=>{s.innerHTML="",s.className="hidden"},document.getElementById("complete-interbank-btn").onclick=()=>{g("IMF",()=>{g("COT",()=>{g("VAT",async()=>{P();try{const i=parseFloat(t.amount),l=parseFloat(t.account.balance),c=l-i;await u.from("accounts").update({balance:c}).eq("id",t.account.id);const{data:m,error:p}=await u.from("transactions").insert([{account_id:t.account.id,user_id:t.profile.id,type:"interbank",description:t.desc,amount:i,balance_before:l,balance_after:c,status:"pending"}]).select().single();await u.from("notifications").insert([{user_id:t.profile.id,title:"Inter-Bank Transfer Initiated",message:`Your inter-bank transfer of ${f(t.amount)} to ${t.accountName} is awaiting admin approval.`,type:"info",read:!1}]),fetch("/api/send-email",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({to:t.profile.email,subject:"Inter-Bank Transfer Receipt",html:y({amount:t.amount,senderName:t.profile.full_name,recipientName:t.accountName,accountNumber:t.accountNum,description:t.desc,fees:"0.00",status:"Pending",referenceNumber:m?.id||x(),companyName:"Assurance Bank",companyAddress:"123 Main St, City, Country",companyPhone:"+1 (555) 123-4567",companyEmail:"assurancebankcc@gmail.com"})})})}catch{o("Failed to process transaction. Please try again.","error")}})})})}}}return{html:`
      <div class="relative">
        ${a.html}
