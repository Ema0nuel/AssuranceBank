import{s as r}from"./supabaseClient-BzAKCqO5.js";import{n as f}from"./Navbar-pMjcO_a8.js";import{r as u}from"./reset-CYKpHJhn.js";import{N as y}from"./logo-nobg-UuYbhWoI.js";import"./toast-DRvdR0y9.js";import"./logo-B-uS21Zh.js";function v(a={}){const t={...{title:"Transaction Receipt",receiptId:a.id||"",date:a.date||new Date().toLocaleDateString(),time:a.time||new Date().toLocaleTimeString(),amount:a.amount||"0.00",currency:"$",description:a.description||"",senderName:a.by||"",recipientName:a.to||"",bankName:a.bankName||"",accountNumber:a.accountNumber||"",transactionType:a.type||"",status:a.status||"Completed",referenceNumber:a.id||"",fees:a.fees||"0.00",totalAmount:a.amount||"",companyName:"Assurance Bank",companyAddress:"123 Main St, City, Country",companyPhone:"+1 (555) 123-4567",companyEmail:"assurancebankcc@gmail.com",additionalFields:a.additionalFields||{},showFooter:!0,footerText:"Thank you for banking with us!"},...a};return`
    <div class="receipt-container font-mono">
      <div class="text-center mb-4">
        <img src="${y}" alt="Assurance Bank" class="h-10 mx-auto mb-2" />
    ...
    `}const j=async()=>{u("Account Summary");const a=f(),c=await r.auth.getSession();if(!c.data.session){window.location.href="/login";return}const{user:t}=c.data.session,{data:o}=await r.from("profiles").select("*").eq("id",t.id).single(),{data:n}=await r.from("accounts").select("*").eq("user_id",t.id).single(),{data:i}=await r.from("transactions").select("*").eq("account_id",n?.id).order("created_at",{ascending:!1}),d=e=>typeof e=="number"?e.toLocaleString("en-US",{style:"currency",currency:"USD",minimumFractionDigits:2}):e||"$0.00",p=i&&i.length?i.map((e,s)=>`
            <tr class="cursor-pointer hover:bg-yellow-50 dark:hover:bg-blue-900/20" data-txid="${e.id}">
                ...
                <td class="px-2 py-1 text-xs font-semibold text-yellow-500">${d(e.amount)}</td>
        ...
        `).join(""):'<tr><td colspan="8" class="text-center text-gray-400 dark:text-gray-500 py-2 text-xs">No transactions found.</td></tr>';function x(){a.pageEvents?.(),document.querySelectorAll("tr[data-txid]").forEach(e=>{e.addEventListener("click",()=>{const s=e.getAttribute("data-txid"),l=i.find(b=>b.id===s);l&&m(l)})})}function m(e){let s=document.getElementById("receipt-modal");s||(s=document.createElement("div"),s.id="receipt-modal",document.body.appendChild(s)),s.className="",s.innerHTML=`
            <div class="fixed inset-0 z-50 flex items-center justify-center bg-black bg-opacity-50 transition-opacity duration-300">
                <div class="receipt-modal-content bg-white dark:bg-gray-900 rounded-xl shadow-2xl w-full max-w-md mx-2 p-0 relative overflow-auto" style="max-height:90vh;">
                    ...
                    <div class="p-6">
                        ${v({id:e.id,date:e.created_at?.slice(0,10),time:e.created_at?.slice(11,16),amount:d(e.amount).replace("$",""),currency:"$",description:e.description,by:e.by,to:e.to,type:e.type,status:e.status,referenceNumber:e.id,fees:e.fees||"0.00",totalAmount:d(e.amount).replace("$",""),additionalFields:{Reason:e.reason||"-","Balance Before":d(e.balance_before),"Balance After":d(e.balance_after)}})}
                    </div>
                </div>
            </div>
            <style>
                    ...
                    <div class="flex flex-col md:flex-row md:items-center md:justify-between mb-4">
                        ...
                        <div class="flex flex-wrap gap-2">
                            <div class="bg-green-100 rounded px-4 py-2 flex items-center gap-2">
                                ...
                                <div>
                                    <div class="text-xs font-semibold text-green-800">${d(n?.balance)}</div>
        ...
        `,pageEvents:x}};export{j as default};
