...
// Update the cardSVG function with better styling and flip animation
function cardSVG({ card_number, card_type, card_holder, expiry, cvv }) {
  ...
  <div class="flex justify-center items-center py-6">
    ...
    `<span class="card-number-group">${group}</span>`
  ).join('')}
            </div>
          </div>
          ...
          <div class="absolute bottom-6 left-6 right-6 flex justify-between items-end">
            <div>
              ...
              <div class="font-medium tracking-wider text-white/90">${card_holder}</div>
            </div>
            <div class="text-right">
              <div class="text-xs text-white/50 uppercase mb-1">Expires</div>
              <div class="font-medium text-white/90">${expiry}</div>
            </div>
          </div>
        </div>
          ...
          <div class="mx-6 mt-8">
            <div class="relative">
              ...
              <div class="w-full h-10 bg-white/90 flex items-center justify-end pr-4">
                <span class="font-mono text-black font-medium">${cvv}</span>
              </div>
            </div>
...
const cards = async () => {
  ...
  function pageEvents() {
    ...
    // Set up flip functionality whenever card is shown
    setupCardFlip();

    // Also set up flip when clicking table rows
    document.querySelectorAll(".js-card-row").forEach((row) => {
      row.onclick = () => {
        ...
        // Set up flip functionality for newly rendered card
        setupCardFlip();
      };
    });
  }
